 ...
 *      of a color name provided. The last game played is a game of
 *      sudoku. The user gets a fixed grid to test the solution. They 
 *      lose points for every wrong number guessed. The score is then 
 *      calculated and if it is higher than the lowest high score the 
 *      user enters their initials and it is recorded in the high scores 
 *      screen.
 *************************************************************************/
package Hangman;
...
public class HangmanGame {
    ...
    private static void createAndShowGUI(HangmanGame hg) {
        ...
        //set the frame to close on pressing escape
        f.getRootPane().getInputMap().put(KeyStroke.getKeyStroke(
        ...
        //set the frame to give group information on pressing f1
        f.getRootPane().getInputMap().put(KeyStroke.getKeyStroke(
                              KeyEvent.VK_F1, 0), "names");
        f.getRootPane().getActionMap().put("names", new AbstractAction() { 
            public void actionPerformed(ActionEvent e) {
                JOptionPane.showMessageDialog(null, 
                        "Tristin Hrafnsson, 009821123\n"
                      + "Daryl Occ, 010475647\n"
                      + "Ashkan Ghafari, 009146475\n"
    ...
    private void presentOptions(JFrame f) {
        ...
        game.setToolTipText("Play a series of 3 games");
        JButton scoresButton = new JButton("HIGH SCORES");
        scoresButton.setToolTipText("Check the high scores");
        JButton credits = new JButton("CREDITS");
        credits.setToolTipText("View the creator credits");
    ...
    private void playGame(JFrame f) {
        ...
        skip.setToolTipText("Skip current game and get 0 points");
    ...
    //method: playGame3
    //purpose: make and add the third game panel to the frame
    private void playGame3(JFrame f) {
        //create a new third game screen and add it to the frame
        GameScreen3 gs3 = new GameScreen3();
        f.add(gs3);
        ...
        //give the initial points for the game
        score += 540;
        gs3.setScore(score);
        
        //create the button to submit a potential solution
        JButton submit = new JButton("SUBMIT");
        submit.setToolTipText("Check if the current solution is correct");
        submit.setBounds(10, 300, 90, 30);
        submit.addMouseListener(new MouseAdapter() { 
            public void mouseClicked(MouseEvent e) {
                //if the solution is correct
                if (gs3.checkSolution()) {
                    //remove the screen and send the final score
                    //to the next method
                    gs3.removeAll();
        ...
        //create a button to quit a game
        JButton quit = new JButton("QUIT");
        quit.setToolTipText("Quit the game");
        quit.setBounds(480, 300, 90, 30);
        quit.addMouseListener(new MouseAdapter() { 
            public void mouseClicked(MouseEvent e) {
                //if they have attempted the game once
                if (gs3.getCheckedOnce()) {
                    //remove the screen but allow the user
                    //to keep their score after 1 incorrect
                    //solution
                    gs3.removeAll();
                    gs3.setVisible(false);
                    score = gs3.getScore();
                  ...
                  //if no attempt has been made
                    ...
                    //take away all 540 points
                    gs3.removeAll();
                    gs3.setVisible(false);
                    getGameEnd(score - 540, f);
                }
            }
        });
        gs3.add(quit);
    ...
    private void getGameEnd(int n, JFrame f) {   
        ...
        end.setToolTipText("End the game and go back to the menu");
        es.setLayout(null);
    ...
    private void getInitials(int n, JFrame f) {
        ...
        JButton end = new JButton("END");
        end.setToolTipText("Submit your initials and go back to the options menu");
        end.setBounds(500, 300, 60, 40);
        end.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                //remove the end screen
    ...
    private void getScores(JFrame f) {
        ...
        back.setToolTipText("Go back to the options screen");
        //set the location of the button
        hss.setLayout(null);
    ...
    private void getCredits(JFrame f) {
        ...
        JButton back = new JButton("BACK");
        back.setToolTipText("Go back to options screen");
